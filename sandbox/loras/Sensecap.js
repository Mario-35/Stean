...
function decode(bytes, nomenclature) {
    ...
    function Decoder (bytes) {
        ...
        for (var forFrame = 0; forFrame < frameArray.length; forFrame++) {
            ...
            switch (realDataValue) {
                case 2000001:
                    decoded["valid"] = false;
                    decoded["err"] = 2000001;
                    decoded["messages"]  = ["SENSOR_NOT_RESPONSE"];
                    return decoded;
                case 2000001:
                    decoded["valid"] = false;
                    decoded["err"] = 2000002;
                    decoded["messages"]  = ["SENSOR_DATA_HEAD_ERROR"];
                    return decoded;
                case 2000001:
                    decoded["valid"] = false;
                    decoded["err"] = 2000003;
                    decoded["messages"]  = ["SENSOR_ARG_INVAILD"];
                    return decoded;
                case 2000001:
                    decoded["valid"] = false;
                    decoded["err"] = 2000257;
                    decoded["messages"]  = ["SENSOR_DATA_ERROR_UNKONW"];
                    return decoded;
            } 
            ...
            if (parseInt(dataID) > 4096) {
                ...
                decoded.datas[nomenclature[dataID]] = realDataValue;
    ...
    function toBinary (arr) {
        return arr.map(hex => ('0'.repeat(8) + (parseInt(hex, 16)).toString(2)).substr(-8)).join("");
    }

    ...
    describe(`{get} ${entity.name} ${nbColorTitle}[9.2]`, () => {
        ...
        it(`Return all Observations in the Datastream that holds the id 2`, (done) => {
            const infos = addTest({
                ...
                request: `${testVersion}/Datastreams(2)/${entity.name}`
            });
            chai.request(server)
                .get(`/test/${infos.request}`)
                .end((err: Error, res: any) => {
        ...
        it("Return Observations with multiple SELECT odata", (done) => {
            const infos = addTest({
                ...
                request: `${testVersion}/${entity.name}(1)?$select=phenomenonTime,result`
            });
            chai.request(server)
                .get(`/test/${infos.request}`)
                .end((err: Error, res: any) => {
        ...
        it("Return Observations with multiple Standard result from multiDatastreams", (done) => {
            const infos = addTest({
                ...
                request: `${testVersion}/${entity.name}(11)`
            });
            chai.request(server)
                .get(`/test/${infos.request}`)
                .end((err: Error, res: any) => {
        ...
        it("Return Observations with multiple result from multiDatastreams", (done) => {
            const infos = addTest({
                ...
                request: `${testVersion}/${entity.name}(378)?$valuesKeys=true`
            });
            chai.request(server)
                .get(`/test/${infos.request}`)
                .end((err: Error, res: any) => {
        ...
        it("Return errors with spliResult on Observation entity Only", (done) => {
            const infos = addTest({
                ...
                description: "",
                reference: "",
                request: `${testVersion}/${entity.name}?$splitResult=ALL`
            });
            chai.request(server)
                .get(`/test/${infos.request}`)
                .end((err: Error, res: any) => {
        ...
        it("Return Observations with multiple result and split results", (done) => {
            const infos = addTest({
                ...
                request: `${testVersion}/MultiDatastreams(1)/${entity.name}?$splitResult=all`
            });
            chai.request(server)
                .get(`/test/${infos.request}`)
                .end((err: Error, res: any) => {
        ...
        it("Return Observations with multiple result and split result soil temperature", (done) => {
            const infos = addTest({
                ...
                request: `${testVersion}/MultiDatastreams(1)/${entity.name}?$splitResult="${testDatas.MultiDatastreams[0].unitOfMeasurements[0].name}"`
            });
            chai.request(server)
                .get(`/test/${infos.request}`)
                .end((err: Error, res: any) => {
    ...
    describe(`{post} ${entity.name} ${nbColorTitle}[10.2]`, () => {
        ...
        it("Return updated from Datastream", (done) => {
            ...
            const infos = addTest({
                ...
                reference: "https://docs.ogc.org/is/18-088/18-088.html#link-existing-entities-when-creating",
                request: `${testVersion}/Datastreams(10)/${entity.name}`,
                params: datas
            });
            chai.request(server)
                .post(`/test/${infos.request}`)
        ...
        it("Return updated Observation from MultiDatastream", (done) => {
            ...
            const infos = addTest({
                ...
                reference: "https://docs.ogc.org/is/18-088/18-088.html#link-existing-entities-when-creating",
                request: `${testVersion}/MultiDatastreams(2)/${entity.name}`,
                params: datas
            });
            chai.request(server)
                .post(`/test/${infos.request}`)

    ...
    describe(`{get} ${entity.name} ${nbColorTitle}[9.2]`, () => {
        ...
        it(`Return all informations for a ${entity.name}.`, (done) => {
            const infos = addTest({
                ...
                request: `${testVersion}/${entity.name}(1)?$expand=Thing/Locations/FeatureOfInterest,Sensor,ObservedProperties`
            });
            chai.request(server)
                .get(`/test/${infos.request}`)
                .end((err: Error, res: any) => {
        ...
        it(`Return ${entity.name} phenomenonTime search`, (done) => {
            const infos = addTest({
                ...
                request: `${testVersion}/${entity.name}?$filter=phenomenonTime eq 2023-03-01T16:30:01Z/2023-03-01T19:15:01Z`
            });
            chai.request(server)
                .get(`/test/${infos.request}`)
                .end((err: Error, res: any) => {
        ...
        it(`Return ${entity.name} phenomenonTime select & order`, (done) => {
            const infos = addTest({
                ...
                request: `${testVersion}/${entity.name}?$select=phenomenonTime&$orderby=phenomenonTime%20desc`
            });
            chai.request(server)
                .get(`/test/${infos.request}`)
                .end((err: Error, res: any) => {
    ...
    describe(`{post} ${entity.name} ${nbColorTitle}[10.2]`, () => {
        ...
        it("Return Error if ObservedProperties length not equal multiObservationDataTypes", (done) => {
            ...
            const infos = addTest({
                ...
                description: "",
                reference: "",
                request: `${testVersion}/MultiDatastreams`
            });
            chai.request(server)
                .post(`/test/${infos.request}`)
                .send(datas)
        ...
        it(`Return updated ${entity.name} ${nbColor}[10.3.1]`, (done) => {
            executeQuery(last(entity.table, true)).then((items: Record<string, any>) => {
                ...
                const infos = addTest({
                    ...
                    request: `${testVersion}/${entity.name}(${items["id"]})`,
                    params: datas
                });
                chai.request(server)

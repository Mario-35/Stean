  			...
  			switch (temp[0]) {
  				...
  				default:
  					...
  					if (element) {
  						switch (element.type) {
  							case "checkbox":
  								getElement(element.id).checked = temp[1] === "true" ? true : false;
  								break;
  ...
  function createUrl() {
  	...
  	if (propertyOption.value != "none" && idOption.value != "") {
  		...
  		if (getElement("onlyValueOption").checked === true) {
  	...
  	addInOption("count", getElement("countOption").checked === true ? "true" : "");
  	addInOption("valuesKeys", getElement("valueskeysOption").checked === true ? "true" : "");

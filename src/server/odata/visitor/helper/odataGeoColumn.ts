    ...
    if (tempEntity) {
      if (this.column.includes(".")) {
        ...
        if (tm && tm.columns.hasOwnProperty(temp[1])) {
          ...
          this.src.subQuery.where = `CASE WHEN "${EConstant.encoding}" LIKE '%geo+json' THEN ST_GeomFromEWKT(ST_GeomFromGeoJSON(coalesce(${doubleQuotes(temp[1])}->'geometry',${doubleQuotes(temp[1])}))) ELSE ST_GeomFromEWKT(${doubleQuotes(temp[1])}::text) END`;
        ...
        if (tempEntity.relations.hasOwnProperty(temp[0])) {
          ...
          if (relation.entity) {
            this.column = `(SELECT ${doubleQuotes(temp[1])} FROM ${doubleQuotes(relation.entity.table)} WHERE ${expand(this.src.ctx,tempEntity.name, temp[0])} AND length(${doubleQuotes(temp[1])}::text) > 2)`;
            if (tempEntity.columns.hasOwnProperty(EConstant.encoding))  test = `(SELECT ${doubleQuotes(EConstant.encoding)} FROM ${doubleQuotes(relation.entity.table)} WHERE ${expand(this.src.ctx,tempEntity.name, temp[0])})`;
        ...
        if (tempEntity.relations.hasOwnProperty(this.column)) {
          ...
          if (relation.entity) {
            this.column = `(SELECT ${doubleQuotes(relation.column)} FROM ${doubleQuotes(relation.entity.table)} WHERE ${expand(this.src.ctx,tempEntity.name, this.column)} AND length(${doubleQuotes(relation.column)}::text) > 2)`;

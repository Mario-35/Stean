...
namespace ResourcePath {
    ...
    export function singleNavigation(value: Utils.SourceArray, index: number, metadataContext?: any): Lexer.Token | undefined {
        let token = ResourcePath.boundOperation(value, index, false, metadataContext) || Expressions.refExpr(value, index) || Expressions.valueExpr(value, index);
        if (token) return token;

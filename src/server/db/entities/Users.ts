...
export class Users extends Common {
  ...
  async getAll(): Promise<IreturnResult | undefined> {
    ...
    if (this.ctx.user?.PDCUAS[EUserRights.SuperAdmin] === true || this.ctx.user?.PDCUAS[EUserRights.Admin] === true ) {
      const temp = await executeSqlValues(config.getService(EConstant.admin), `SELECT ${models.getSelectColumnList(this.ctx.service, models.DBAdmin(config.getService(EConstant.admin)).Users, true)} FROM "${USER.table}" ORDER BY "id"`);      

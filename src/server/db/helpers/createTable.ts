 ...
 */
export const createTable = async ( serviceName: string, tableEntity: Ientity, doAfter: string | undefined ): Promise<IKeyString> => {
  ...
  if (tableEntity.table.trim() != "")
    ...
    await config.connection(serviceName).unsafe(sql)
        .then(() => EChar.ok)
  ...
  if (tableEntity.after) {
    ...
    if (tableEntity.after.toUpperCase().startsWith("INSERT"))
      ...
      await config.connection(serviceName).unsafe(tableEntity.after)
          .then(() => EChar.ok)
          .catch((error: Error) => {
            log.error(error);
  ...
  if (doAfter) {
    log.query(doAfter);
    returnValue[`${tab()} doAfter ${tableEntity.table}`] = await config.connection(serviceName).unsafe(doAfter)
      .then(() => EChar.ok)
      .catch((error: Error) => error.message);
  }

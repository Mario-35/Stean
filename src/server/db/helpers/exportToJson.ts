    ...
    await asyncForEach(
        ...
        async (entity: string) => {
            if (Object.keys(ctx.model[entity].columns) && ctx.model[entity].table != "") {
                ...
                // Execute query
                const tempResult = await config.connection(ctx.service.name).unsafe(`select ${columnListWithQuotes}${rels.length > 1 ? rels.join() : ""}${EConstant.return} FROM "${ctx.model[entity].table}" LIMIT ${getUrlKey(ctx.request.url, "limit") || ctx.service.nb_page}`);

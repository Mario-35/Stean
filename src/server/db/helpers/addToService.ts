    ...
    if (temp) {
        ...
        await asyncForEach(dataInput["value"], async (line: Record<string, any>) => {
            if (line["payload"] != "000000000000000000")
                try {
                    const datas = line["value"] ? { "timestamp": line["phenomenonTime"], "value": line["value"], "deveui": line["deveui"].toUpperCase() } : { "timestamp": line["phenomenonTime"], "frame": line["payload"].toUpperCase(), "deveui": line["deveui"].toUpperCase() };

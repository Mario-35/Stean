...
export const createService = async (ctx: koaContext, dataInput: Record<string, any>): Promise<Record<string, any>> => {
  ...
  await config.executeAdmin(sqlStopDbName(simpleQuotesString(serviceName))).then(async () => {
    await config.executeAdmin(`DROP DATABASE IF EXISTS ${serviceName}`).then(async () => {
    ...
    if (dataInput[entityName]) {
      ...
      if (goodEntity) {
        try {
          ...
          await config.executeSqlValues(config.getService(serviceName), sqls.join(";")).then((res: Record<string, any>) =>{

...
export const createService = async (ctx: koaContext, dataInput: Record<string, any>): Promise<Record<string, any>> => {
  ...
  await executeAdmin(sqlStopDbName(simpleQuotes(serviceName))).then(async () => {
  ...
  await asyncForEach( Object.keys(tmp) .filter((elem: string) => tmp[elem].createOrder > 0) .sort((a, b) => (tmp[a].createOrder > tmp[b].createOrder ? 1 : -1)), async (entityName: string) => {
    if (dataInput[entityName]) {
      ...
      if (goodEntity) {
        try {
          const sqls: string[] =dataInput[entityName].map((element: any) =>`INSERT INTO ${doubleQuotes(goodEntity.table)} ${createInsertValues(ctx, prepareDatas(element, goodEntity.name), goodEntity.name)}`);

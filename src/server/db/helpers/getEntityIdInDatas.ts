...
export const getEntityIdInDatas = (dataInput: Record<string, string>, pg : RootPgVisitor) => {
    const res: { [key: string]: BigInt | undefined } = {};
    if (pg.entity) Object.keys(pg.entity.relations).forEach((elem) => {        
      if (dataInput[elem]) {
        res[elem] = dataInput[elem] && dataInput[elem] != null && dataInput[elem][EConstant.id as keyof object]

 ...
 * @review 27-01-2024
 * @author formatPostgresString.adam@inrae.fr
 *
 */
// onsole.log("!----------------------------------- asDataArray. -----------------------------------!");
import { asJson } from ".";
import { ESCAPE_SIMPLE_QUOTE, _COLUMNSEPARATOR, _NEWLINE } from "../../constants";
import { doubleQuotesString, simpleQuotesString, formatPostgresString } from "../../helpers";
import { PgVisitor } from "../../odata/visitor";

export const asDataArray = (input: PgVisitor): string => {  
  
  const names:string[] = input.toPgQuery()?.keys.map((e: string) => formatPostgresString(e)) || [];
  // create names

...
class Queries {
    ...
    createRowNumber(element: PgVisitor, offset: number, limit: number, order?: string) {    
       return this.removeReturn(`id IN ( SELECT id FROM ( SELECT id, ROW_NUMBER() OVER () FROM ( SELECT id FROM ${element.partitioned?.from.split(" AS ")[0].trim()}${order ? ` ORDER BY "${order}"` : '@ORDERBY@'}${offset && offset > 0 ? ` OFFSET ${offset}` : ''}${limit && limit >= 0 ? ` LIMIT ${limit}` : ''} )))`);
    }

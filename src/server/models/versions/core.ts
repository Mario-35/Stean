...
import { doubleQuotes, isTest, formatPgTableColumn, isString, hidePassword } from "../../helpers";
...
import { Numeric, Text } from "../types";
...
export class Core {
    ...
    public async streamInfos(ctx: koaContext, input: Record<string, any>): Promise<IstreamInfos | undefined> {
        ...
        const streamEntity = this.entityName(ctx._.model(), stream);
        if (!streamEntity) return undefined;
        const foiId: Id = input["FeaturesOfInterest"] ? input["FeaturesOfInterest"] : undefined;
        const searchKey = input[this.getModel(ctx._.service)[streamEntity].name] || input[this.getModel(ctx._.service)[streamEntity].singular];
        const streamId: Id = isNaN(searchKey) ? searchKey[EConstant.id] : searchKey;
        if (streamId) {
            return executeSqlValues(
                ctx._.service,
                queries.asJson({
                    query: `SELECT "id", "observationType", "_default_featureofinterest" FROM ${doubleQuotes(this.getModel(ctx._.service)[streamEntity].table)} WHERE "id" = ${BigInt(
                        streamId
                    )} LIMIT 1`,
                    singular: true,
                    strip: false,
    ...
    public makeModel(entities: Ientity[], service?: Iservice) {
    ...
    public getModel(service: Iservice | string): Ientities {
        ...
        if (!Core.models[service.name]) Core.models[service.name] = this.create(service.apiVersion, service);
        return Core.models[service.name];
    }

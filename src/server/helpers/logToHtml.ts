...
export function logToHtml(input: string): string {
    if (input) {
        if (typeof input !== "string") process.stdout.write(util.inspect(input, { showHidden: false, depth: null, colors: false }) + EConstant.return);
        input = String(input);
        ...
        return input
            .split(/\r?\n/)
            .map((e: string) => (e.trim().startsWith("</span>") ? `${formate(e)}</span><br />\r` : `${e}\r`))
            .join("");
    }
    return "";
}

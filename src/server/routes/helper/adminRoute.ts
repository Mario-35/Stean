...
export const adminRoute = async (ctx: koaContext) => {
  ctx.set("script-src", "self");
  ctx.set("Content-Security-Policy", "self");
  ctx.type = returnFormats.html.type;
  ctx.body = new Admin(ctx, { connection: await adminConnectPg(ctx), url: ctx.request.url, body: ctx.request.body, why: {} }).toString();
}